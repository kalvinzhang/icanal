...
func (c *simpleConnector) doConnect() error {
	...
	ca := &protocol.ClientAuth{
		...
		NetReadTimeoutPresent:  &protocol.ClientAuth_NetReadTimeout{NetReadTimeout: int32(c.config.IdleTimeout.Milliseconds())},
		NetWriteTimeoutPresent: &protocol.ClientAuth_NetWriteTimeout{NetWriteTimeout: int32(c.config.IdleTimeout.Milliseconds())},
	}
	data := marshalPacketIgnoreError(protocol.PacketType_CLIENTAUTHENTICATION, ca)
...
func (c *simpleConnector) handshake() (*protocol.Handshake, error) {
	packet, err := c.readNextPacket(nil)
	if err != nil {
		return nil, err
	}
...
func (c *simpleConnector) Subscribe(_ context.Context, filter string) error {
	...
	data := marshalPacketIgnoreError(protocol.PacketType_SUBSCRIPTION, &protocol.Sub{
...
func (c *simpleConnector) GetWithoutAck(ctx context.Context, batchSize int32, timeout time.Duration) (*Message, error) {
	...
	get := &protocol.Get{
		...
		ClientId:       strconv.Itoa(c.clientIdentity.ClientId),
		FetchSize:      batchSize,
		TimeoutPresent: &protocol.Get_Timeout{Timeout: innerTimeout},
		UnitPresent:    &protocol.Get_Unit{Unit: int32(TimeUnitMilliseconds)},
		AutoAckPresent: &protocol.Get_AutoAck{AutoAck: false},
	}
	data := marshalPacketIgnoreError(protocol.PacketType_GET, get)
...
func (c *simpleConnector) Rollback(ctx context.Context, batchId int64) error {
	...
	data := marshalPacketIgnoreError(protocol.PacketType_CLIENTROLLBACK, &protocol.ClientRollback{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
		BatchId:     batchId,
	})
...
func (c *simpleConnector) readNextPacket(ctx context.Context) (*protocol.Packet, error) {
...
func (c *simpleConnector) Unsubscribe(context.Context) error {
	...
	data := marshalPacketIgnoreError(protocol.PacketType_UNSUBSCRIPTION, &protocol.Unsub{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
	})
...
func (c *simpleConnector) Ack(ctx context.Context, batchId int64) error {
	...
	data := marshalPacketIgnoreError(protocol.PacketType_ACK, &protocol.ClientAck{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
		BatchId:     batchId,
	})
