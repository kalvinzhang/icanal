...
func (c *simpleConnector) doConnect() error {
	...
	ca := &ClientAuth{
		...
		NetReadTimeoutPresent:  &ClientAuth_NetReadTimeout{NetReadTimeout: int32(c.config.IdleTimeout.Milliseconds())},
		NetWriteTimeoutPresent: &ClientAuth_NetWriteTimeout{NetWriteTimeout: int32(c.config.IdleTimeout.Milliseconds())},
	}
	data := marshalPacketIgnoreError(PacketType_CLIENTAUTHENTICATION, ca)
	...
	if packet.GetType() != PacketType_ACK {
...
func (c *simpleConnector) handshake() (*Handshake, error) {
	packet, err := c.readNextPacket(nil)
	if err != nil {
		return nil, err
	}
	...
	if packet.GetType() != PacketType_HANDSHAKE {
...
func (c *simpleConnector) Subscribe(_ context.Context, filter string) error {
	...
	data := marshalPacketIgnoreError(PacketType_SUBSCRIPTION, &Sub{
...
func (c *simpleConnector) GetWithoutAck(ctx context.Context, batchSize int32, timeout time.Duration) (*Message, error) {
	...
	get := &Get{
		...
		ClientId:       strconv.Itoa(c.clientIdentity.ClientId),
		FetchSize:      batchSize,
		TimeoutPresent: &Get_Timeout{Timeout: innerTimeout},
		UnitPresent:    &Get_Unit{Unit: int32(TimeUnitMilliseconds)},
		AutoAckPresent: &Get_AutoAck{AutoAck: false},
	}
	data := marshalPacketIgnoreError(PacketType_GET, get)
...
func (c *simpleConnector) Rollback(ctx context.Context, batchId int64) error {
	...
	data := marshalPacketIgnoreError(PacketType_CLIENTROLLBACK, &ClientRollback{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
		BatchId:     batchId,
	})
...
func (c *simpleConnector) readNextPacket(ctx context.Context) (*Packet, error) {
...
func (c *simpleConnector) Unsubscribe(context.Context) error {
	...
	data := marshalPacketIgnoreError(PacketType_UNSUBSCRIPTION, &Unsub{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
	})
...
func (c *simpleConnector) Ack(ctx context.Context, batchId int64) error {
	...
	data := marshalPacketIgnoreError(PacketType_ACK, &ClientAck{
		Destination: c.clientIdentity.Destination,
		ClientId:    strconv.Itoa(c.clientIdentity.ClientId),
		BatchId:     batchId,
	})
